...
LOG = logging.getLogger('ryu.app.topology_rest')
...
class TopologyRestController(ControllerBase):
    ...
    def list_switches(self, req, **kwargs):
        LOG.info("Received request for /v1.0/topology/switches")
        switches = get_switch(self.topology_api_app, None)
        body = json.dumps([{"dpid": "{:016x}".format(switch.dp.id)} for switch in switches])
        return Response(content_type='application/json', body=body)

    def get_switch(self, req, **kwargs):
        LOG.info(f"Received request for /v1.0/topology/switches/{kwargs['dpid']}")
    ...
    def list_links(self, req, **kwargs):
        LOG.info("Received request for /v1.0/topology/links")
        links = get_link(self.topology_api_app, None)
        body = json.dumps([{
            "src": "{:016x}".format(link.src.dpid),
            "src_port": link.src.port_no,
            "dst": "{:016x}".format(link.dst.dpid),
            "dst_port": link.dst.port_no
        } for link in links])
        return Response(content_type='application/json', body=body)

    def get_links(self, req, **kwargs):
        LOG.info(f"Received request for /v1.0/topology/links/{kwargs['dpid']}")
        dpid = dpid_lib.str_to_dpid(kwargs['dpid'])
    ...
    def list_hosts(self, req, **kwargs):
        LOG.info("Received request for /v1.0/topology/hosts")
        hosts = get_host(self.topology_api_app, None)
        body = json.dumps([{
            "mac": host.mac,
            "ip": host.ipv4,
            "dpid": "{:016x}".format(host.port.dpid),
            "port_no": host.port.port_no
        } for host in hosts])
        return Response(content_type='application/json', body=body)

    def get_hosts(self, req, **kwargs):
        LOG.info(f"Received request for /v1.0/topology/hosts/{kwargs['dpid']}")
        dpid = dpid_lib.str_to_dpid(kwargs['dpid'])
...
class RESTApp(app_manager.RyuApp):
    ...
    _CONTEXTS = {
        ...
        'switches': switches.Switches  # Enables topology discovery
    }

    def __init__(self, *args, **kwargs):
        super(RESTApp, self).__init__(*args, **kwargs)
        self.dpset = kwargs['dpset']
        self.switches = kwargs['switches']  # Store topology discovery
        wsgi = kwargs['wsgi']
        ...
        self.data = {
            ...
            'topology_api_app': self  # This is required for topology discovery
        }

        wsgi.registory[MyStatsController.__name__] = self.data
        wsgi.registory[TopologyRestController.__name__] = self.data
        LOG.info("Registered Controllers: %s", wsgi.registory.keys())
    ...
    def _setup_routes(self, mapper):
      ...
      # --- Rutele pentru API-ul OpenFlow (StatsController) ---
        ...
        path_topo = '/v1.0/topology'

        mapper.connect('topology', f'{path_topo}/switches',
                    controller=TopologyRestController, action='list_switches',
                    conditions=dict(method=['GET']))

        mapper.connect('topology', f'{path_topo}/switches/{{dpid}}',
                    controller=TopologyRestController, action='get_switch',
                    conditions=dict(method=['GET']))

        mapper.connect('topology', f'{path_topo}/links',
                    controller=TopologyRestController, action='list_links',
                    conditions=dict(method=['GET']))

        mapper.connect('topology', f'{path_topo}/links/{{dpid}}',
                    controller=TopologyRestController, action='get_links',
                    conditions=dict(method=['GET']))

        mapper.connect('topology', f'{path_topo}/hosts',
                    controller=TopologyRestController, action='list_hosts',
                    conditions=dict(method=['GET']))

        mapper.connect('topology', f'{path_topo}/hosts/{{dpid}}',
                    controller=TopologyRestController, action='get_hosts',
                    conditions=dict(method=['GET']))
    ...
    def switch_enter_handler(self, ev):
        switch = ev.switch
        LOG.info(f"Switch entered: {switch.dp.id:016x}")

    @set_ev_cls(event.EventSwitchLeave)
    def switch_leave_handler(self, ev):
        switch = ev.switch
        LOG.info(f"Switch left: {switch.dp.id:016x}")

    @set_ev_cls(event.EventLinkAdd)
    def link_add_handler(self, ev):
        link = ev.link
        LOG.info(f"Link added: {link.src.dpid:016x}:{link.src.port_no} -> {link.dst.dpid:016x}:{link.dst.port_no}")

    @set_ev_cls(event.EventLinkDelete)
    def link_delete_handler(self, ev):
        link = ev.link
        LOG.info(f"Link deleted: {link.src.dpid:016x}:{link.src.port_no} -> {link.dst.dpid:016x}:{link.dst.port_no}")
